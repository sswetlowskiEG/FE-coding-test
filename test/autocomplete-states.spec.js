...
function filterStates(textEntered) {
		...
		if(textEntered===""||textEntered===null||!textEntered) return [];
		let prioritizedStates = []; 
		states.sort((a,b) => (a.name > b.name) ? 1 : ((b.name > a.name) ? -1 : 0))
	
	// 	Match exact abbreviation (Should be case insensitive)
		states.forEach(function(state){
			if(state.abbrev.toLowerCase() === textEntered.toLowerCase()) {
				prioritizedStates.push(state.name);
				return;
			}
		});
		
	// 	match start of state name
		states.forEach(function(state){
			if(state.name.substring(0, textEntered.length).toLowerCase()==textEntered.toLowerCase()) {
				prioritizedStates.push(state.name);
			}		
		});
	
	// 	match part of state name
		states.forEach(function(state){
		...
		return(Array.from(new Set(prioritizedStates)));
